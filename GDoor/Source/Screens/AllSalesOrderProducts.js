...
import AllSalesOrderContainer from './AllSalesOrderContainer';
import SearchBar from '../CustomComponent/searchBar';
import Header from '../CustomComponent/Header';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';

export default function AllSalesOrderProducts({navigation}) {
  ...
  const getsaleorder = () => {
    ...
    POST('saleorder', raw)
      .then(async response => {
        console.log('saleorder ', response);
        setLoading(false);
  ...
  const renderItem = ({item}) => (
    <AllSalesOrderContainer item={item} navigation={navigation} />
  );
  return (
